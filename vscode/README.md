...
1. **Find the file.** After building (or downloading), you'll have a file called `cc-ninja-0.1.0.vsix`.
...
```
code --install-extension path/to/cc-ninja-0.1.0.vsix
```

Replace `path/to/` with wherever the file actually is. For example:

```
code --install-extension S:\Projects\cc-ninja\cc-ninja-0.1.0.vsix
```
